...
/**
 * Abstract base class for all domain-specific exceptions with automatic error ID generation.
 * This class provides unique exception tracking through UUID generation and automatic
 * stack trace logging to correlate client-facing error IDs with server logs.
 *
 * When chaining exceptions, attempts to preserve the original error ID for better
...
public abstract class ReasonBasedException extends RuntimeException {
	 ...
	 * Generates a stack trace showing the call chain up to the specified depth.
	 * This is used for logging error context without overwhelming log output.
	 *
	 * @return formatted stack trace showing class, method, and file location
	 */
	private static String whereAmI() {
		...
		// Build stack trace string from top of stack (most recent calls)
