...
@Testcontainers
class SecurosysSimpleSignLifecycleTSBTest {
    ...
    private static void configureTsbHsmKeyProvider() throws Exception {
        HsmYamlConfig.Tsb tsb = YamlLoader.loadConfig("tsb-config-RS256.yaml").getTsb();
        ...
        config.put("tsbUrl", Collections.singletonList(tsb.getTsbUrl()));
        config.put("authMethod", Collections.singletonList(tsb.getAuth()));
        putIfPresent(config, "bearerToken", tsb.getBearerToken());
        putIfPresent(config, "mtlsP12Path", tsb.getMtlsP12Path());
        putIfPresent(config, "mtlsP12Password", tsb.getMtlsP12Password());
        putIfPresent(config, "keyOperationApiKey", tsb.getKeyOperationApiKey());
        putIfPresent(config, "keyManagementApiKey", tsb.getKeyManagementApiKey());
        config.put("keyLabel", Collections.singletonList(tsb.getKeyLabel()));
        config.put("keyPassword", Collections.singletonList(tsb.getKeyPassword()));
        config.put("algorithm", Collections.singletonList(tsb.getAlgorithm()));
    ...
    private static void putIfPresent(MultivaluedHashMap<String, String> config, String key, String value) {
